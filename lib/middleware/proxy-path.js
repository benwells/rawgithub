...
@param {String} rootUrl
    Upstream URL to relay requests to (e.g. "https://raw.github.com").
...
module.exports = function (rootUrl) {
    ...
    return function (req, res, next) {
        ...
        https.get({
            ...
            path    : parsedUrl.path + req.path + '?' + qs.stringify(req.query),
            headers : headers
...
function onResponse(req, res, next, incoming) {
    ...
    if ((encoding === 'gzip' || encoding === 'deflate') && !accepts(req).encodings(encoding)) {
